...
const makeAddAccount = (): AddAccount => {
  class AddAccountStub implements AddAccount {
    add (account: AddAccountModel): AccountModel {
      const fakeAccount = { id: 'valid_id', name: 'valid_name', email: 'valid_mail@mail.com', password: 'valid_password' }
      return fakeAccount
    }
  }
...
describe('SignUp Controller', () => {
  ...
  test('Should returns 400 if no passwordConfirmation is provided', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should returns 400 if an invalid e-mail is provided', () => {
  ...
  test('Should returns 400 if passwordConfirmation fails', () => {
    const { sut } = makeSut()
  ...
  test('Should returns 201 when valid data is provided', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'valid_name',
